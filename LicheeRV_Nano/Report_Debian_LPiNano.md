# Debian LicheeRV Nano 系统版本和工具链测试报告
...
```
开机后等待约一分钟左右（取决于存储卡速度），开发板本身会作为RNDIS虚拟网卡连接到电脑，IP为10.*.*.*，Linux下可打开终端运行ip a查看；例如，电脑一侧获取到的IP为10.138.201.107，则开发板本身的IP为：10.138.201.1（最后一位替换为1），通过此IP可连接到开发板SSH：ssh debian@10.138.201.1

默认用户名为：debian，密码为：rv
```

登录系统后，执行以下命令连接Wi-Fi并重启开发板（将wifi_name和password替换成实机Wi-Fi名称以及密码）
```
sudo touch /boot/wifi.sta

sudo sh -c 'echo wifi_name > /boot/wifi.ssid'

sudo sh -c 'echo password > /boot/wifi.pass'
...
[![asciicast](https://asciinema.org/a/vgDtUYicopIheezW.svg)](https://asciinema.org/a/vgDtUYicopIheezW)
## 预期结果
