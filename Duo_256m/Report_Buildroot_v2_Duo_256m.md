# BuildRoot (v2) Milk-V Duo 256M  系统版本和工具链测试报告

## 测试环境

### 操作系统信息

- 系统版本：Duo 256M v2.0.1
- 下载链接：<https://github.com/milkv-duo/duo-buildroot-sdk-v2/releases/download/v2.0.1/milkv-duo256m-musl-riscv64-sd_v2.0.1.img.zip>
...
用 dd 刷写镜像到 sd 卡：
```shell
sudo dd if=milkv-duo256m-musl-riscv64-sd_v2.0.1.img  of=/path/to/your/device bs=4M status=progress
```
### 登录系统
#### 通过串口连接
将 microSD 卡插入 Milk-V Duo 256M，重启。

从右往左数，第一个箭头指的为GND，第二个箭头指的为RX，第三个箭头指的为TX。（连接方式为：开发板GND->调试器GND，开发板TX->调试器RX，开发板RX->调试器TX)
